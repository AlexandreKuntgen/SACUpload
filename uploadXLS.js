...
(function () {
  ...
  class UploadXLS extends HTMLElement {
    constructor() {
      ...
      this._isMasterDataUpload = false;
      this._isProcessData = false;
    ...
    async uploadPublicDimensionMembers(
      dimensionId,
      mapping,
      defaultValues,
      sheetName
    ...
    async createJobAndPostMasterData(data) {
      let jobId;
      let jobURL;
      let runJobURL;
      let validateJobURL;
      ...
      let failedNumberRows;
      let status;
      let dimensionId=this._dimensionId;
      this._uploadResult = {
        currentStep: "Initialization",
        jobId: "",
        baseURL: "",
        modelId: this._dimensionId,
        mapping: this._mapping,
        defaultValues: this._defaultValues,
        data: data,
        httpStatus: "",
      ...
      try {
        this._uploadResult.currentStep = "Creating Job";
        console.log(this._uploadResult.currentStep);
        let jobCreationResponse = await this.createMasterDataJob(dimensionId);
        this.dispatch("onJobCreated");
        jobId = jobCreationResponse.jobID;
        jobURL = jobCreationResponse.jobURL;
        this._uploadResult.jobId = jobId;
      } catch (error) {
        console.log(this._uploadResult);
        this._uploadResult.jobStatus = "FAILED";
      ...
      this.dispatch("onDataLoadToJob");
      //validate the job
      try {
        this._uploadResult.currentStep = "Validating Job";
        console.log(this._uploadResult.currentStep);
        let validateJobResponse = await this.validateJob(validateJobURL);
        this.dispatch("onValidateJob");
        invalidRowsURL = validateJobResponse.invalidRowsURL;
        totalNumberRowsInJob = validateJobResponse.totalNumberRowsInJob;
        failedNumberRows = validateJobResponse.failedNumberRows;
        this._uploadResult.failedNumberRows =
    ...
    async createMasterDataJob(dimensionId) {
      let url =
        this._baseURL +
        "/api/v1/dataimport/publicDimensions/" +
        dimensionId +"/publicDimensionData";
      let options = {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    dispatch(event, arg) {
      ...
      if (event === "onFileUpload" && this._callPostData) {
        ...
        }else if(this._isMasterDataUpload){
          this.createJobAndPostMasterData(data);
        }else if(this._isProcessData){
            this._isProcessData=false;
            return data;
        }else{
